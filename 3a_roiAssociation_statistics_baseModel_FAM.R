...
# adjusting for age, sex, ICV (for volume), and genotype PCs, and any 
# cohort-specific covariates.
#
#*****************************************************************************#
op <- options(warn=1)
logfile=file.path(outdir,"3a_roiAssociation_statistics_baseModel_FAM.log"); 
if(file.exists(logfile)) file.remove(logfile)

starting.message=paste0("\n3a. Start obtaining association estimates for adiposity vs. roi-ctx for the base model (family data).\n")
...
msg = paste0('\n************\n3a: Removed variables due to 100% missing or no variations: ', 
             paste0(rm.var,collapse = ", "),".\n************\n")
cat(msg,sep='')
cat(msg,sep='',file=logfile,append=T)
...
for(adiposity in intersect(c("BMI","waist","WHR"),names(df))){
  ...
  for(pheno in c("volume","thickness","area")){
    ...
    ggsave(file.path(outdir,
                     paste0('roi_assoc_res_two_methods_baseModel_',adiposity,"_ctx_",pheno,'.png')),
...
#ending ----
ending.message = paste0("\nFinished obtaining association estimates for ",adiposity," vs. roi-ctx_",pheno," under base model in ",group,".\n")
