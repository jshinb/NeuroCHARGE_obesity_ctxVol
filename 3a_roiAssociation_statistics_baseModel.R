#*****************************************************************************#
#
# 3a: Examine associations between adiposoty vs. ctx structures across the 34 regions
# adjusting for age, sex, ICV (for volume), and genotype PCs, and any cohort-
# specific covariates.
#
#*****************************************************************************#
op <- options(warn=1)
logfile=file.path(outdir,"3a_roiAssociation_statistics_baseModel.log"); 
if(file.exists(logfile)) file.remove(logfile)

starting.message=paste0("\n3a. Start obtaining association estimates for adiposity vs. roi-ctxTH for the base model.\n")
...
rm.var = c(names(col.SD)[is.na(col.SD)|col.SD==0],monoVar,missingCol)
rm.var = unique(rm.var)
...
cov_ctxSA = setdiff(c('age_mri','age_diff','sex','ICV',cov_genoPC,cov_cohort_specific),rm.var)
cov_ctxTH = setdiff(c('age_mri','age_diff','sex',cov_genoPC,cov_cohort_specific),rm.var)
...
if(!is.null(phist_BMI)){
  cov_BMI = setdiff(c('age_adiposity','age_diff','sex',cov_genoPC,cov_cohort_specific),rm.var)
  adj_BMI = get_adj_BMI(d=df,covariate_names = cov_BMI,remove.ID = F,logfile=logfile)
}else{
  cov_BMI <- adj_BMI <- c()
}
if(!is.null(phist_waist)){
  cov_WC = setdiff(c('age_adiposity','age_diff','sex','height',cov_genoPC,cov_cohort_specific),rm.var)
  adj_WC = get_adj_WC(d=df,covariate_names = cov_WC,remove.ID = F,logfile=logfile)
}else{
  cov_WC <- adj_WC <- c()
}
if(!is.null(phist_WHR)){
  cov_WHR = setdiff(c('age_adiposity','age_diff','sex',cov_genoPC,cov_cohort_specific),rm.var)
...
for(adiposity in intersect(c("BMI","waist","WHR"),names(df))){
  ...
  for(pheno in c("volume","thickness","area")){
    ...
    names(adj_adiposity) = str_replace( names(adj_adiposity),"WC","waist")
    
    ##* approach 1 ----
    assoc_res_adj = c()
    for(roii in c(roi.34,'global') ){
      
      analdati = df %>% dplyr::select(ID,sex) %>%
        left_join(adj_adiposity,join_by(ID)) %>% 
        left_join(
          subset(
            adj_ctx,select = c('ID',paste0(roii,".adjSex"),paste0(roii,".adj"))
          ),
          join_by(ID))
      ...
      ## female
      fit.assoc.F = lm(y~x,data=analdati,subset=sex=="F", na.action = na.exclude)
      ...
      assoc_res_adj_roii.F$mod = "y~x"
      
      ## male
      fit.assoc.M = lm(y~x,data=analdati,subset=sex=="M", na.action = na.exclude)
      ...
      assoc_res_adj_roii.M$mod = "y~x"
      
      # sex-combined
      fit.assoc.sex_combined = lm(y~x+sex,data=analdati,na.action = na.exclude)
      ...
      assoc_res_adj_roii.sex_combined$mod = "y~x"
      
      # sex-by-adiposity interaction effect
      fit.assoc = lm(y~sex*x,data=analdati,na.action = na.exclude)
      ...
      assoc_res_adj_roii$mod = "y~sex*x"
    ...
    save(res1,res2,roi_assoc_res_two_methods,
         file=file.path(outdir,
                        paste0("roi_assoc_res_two_methods_baseModel_",adiposity,"_ctx_",pheno,".Rdata"))
    )
    ...
    png(file.path(outdir,
                  paste0('roi_assoc_res_two_methods_baseModel',adiposity,"_ctx_",pheno,'.png')),
    ...
    ggsave(file.path(outdir,
                  paste0("forest_plot_assoc_",adiposity,"_roi_ctx_",pheno,"_baseModel.png")),
...
#ending ----
ending.message = paste0("\n3a. Finished obtaining association estimates for ",adiposity," vs. roi-ctx_",pheno," under base model in ",group,".\n")
